#pragma once

/**
 * Name: Satisfactory
 * Version: Early_Access_CL#264901
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.SetHiddenInfo
	 */
	struct UWidget_MAMTree_NodeInfo_C_SetHiddenInfo_Params
	{	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.GetResearchButtonFeedback
	 */
	struct UWidget_MAMTree_NodeInfo_C_GetResearchButtonFeedback_Params
	{	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.UpdateState
	 */
	struct UWidget_MAMTree_NodeInfo_C_UpdateState_Params
	{	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.SetSchematic
	 */
	struct UWidget_MAMTree_NodeInfo_C_SetSchematic_Params
	{
	public:
		class UClass*                                              schematic;                                               // 0x0000(0x0008)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.Construct
	 */
	struct UWidget_MAMTree_NodeInfo_C_Construct_Params
	{	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.BndEvt__mResearchButton_K2Node_ComponentBoundEvent_0_OnClicked__DelegateSignature
	 */
	struct UWidget_MAMTree_NodeInfo_C_BndEvt__mResearchButton_K2Node_ComponentBoundEvent_0_OnClicked__DelegateSignature_Params
	{	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.ExecuteUbergraph_Widget_MAMTree_NodeInfo
	 */
	struct UWidget_MAMTree_NodeInfo_C_ExecuteUbergraph_Widget_MAMTree_NodeInfo_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function Widget_MAMTree_NodeInfo.Widget_MAMTree_NodeInfo_C.OnClicked__DelegateSignature
	 */
	struct UWidget_MAMTree_NodeInfo_C_OnClicked__DelegateSignature_Params
	{	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
